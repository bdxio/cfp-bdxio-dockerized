...
docker run --rm -d -p 80:80 -v /var/run/docker.sock:/tmp/docker.sock jwilder/nginx-proxy
...
esContainerName=$(docker --debug run --rm -d -p 49200:9200 -p 49300:9300 $USERNAME/cfp-elasticsearch)
...
docker run --rm -d --link $esContainerName:es --link $redisProdContainerName:redis -e "VIRTUAL_HOST=cfp.bdx.io" -v $CURRENT_DIR/cfp-src-prod:/cfp-src/ $USERNAME/cfp-webapp
docker run --rm -d --link $esContainerName:es --link $redisTestingContainerName:redis -e "VIRTUAL_HOST=cfp-testing.bdx.io" -v $CURRENT_DIR/cfp-src-testing:/cfp-src/ $USERNAME/cfp-webapp

docker run --rm -dv $CURRENT_DIR/dropbox:/Dropbox my-dropbox /.dropbox-dist/dropboxd
